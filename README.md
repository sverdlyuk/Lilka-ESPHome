# ESPHome LVGL для Lilka

ESPHome конфігурація для Lilka на базі ESP32-S3 з підтримкою графічного фреймворку LVGL (LittlevGL).

![IMG_2284](https://github.com/user-attachments/assets/330f7477-2f81-4e32-b60c-75d941f841c9)

---


## 🚀 Можливості

Ця конфігурація забезпечує наступний функціонал:

* **Графічний Інтерфейс (LVGL):**
    * ✅ Підтримка дисплея ST7789V (280x240, SPI).
    * ✅ Інтеграція [LVGL](https://esphome.io/components/lvgl/) для створення інтерактивного інтерфейсу.
    * ✅ [Flex Layout](https://esphome.io/cookbook/lvgl/#flex-layout-positioning) для адаптивного розташування елементів.
    * ✅ Динамічне відображення **стану бойлера (Home Assistant Switch)**.
    * ✅ Відображення **рівня заряду та напруги батареї** з автооновленням.

# 🚀 Основні Мережеві Функції ESPHome

* **OTA (Over-The-Air):** Дозволяє оновлювати прошивку пристрою бездротово через Wi-Fi після першого підключення.
* **Web Server:** Надає веб-інтерфейс (зазвичай на порту 80) для моніторингу статусу, перегляду логів та базового керування пристроєм через браузер.
* **Captive Portal:** Створює аварійну Wi-Fi точку доступу для введення нових облікових даних мережі, якщо пристрій не може підключитися.
* **mDNS (Multicast DNS):** Дозволяє знаходити пристрій у локальній мережі за зрозумілим іменем (`lilka.local`), а не лише за IP-адресою.
* **Native API:** Встановлює швидкий, зашифрований та оптимізований двосторонній зв'язок для інтеграції з Home Assistant.
* **BLE Tracker:** Активує модуль **Bluetooth Low Energy** для сканування та відстеження навколишніх BLE-пристроїв.
* **Bluetooth Proxy:** Передає дані, зібрані BLE Tracker, до Home Assistant, розширюючи покриття мережі розумного будинку.

# 🏡 Керування пристроями Home Assistant (HA)

* **Двосторонній Зв'язок:** ESPHome може отримувати статус та [керувати пристроями](https://esphome.io/components/sensor/homeassistant/) Home Assistant.
* **Налаштування HA:** Для роботи функції керування в інтеграції ESPHome у Home Assistant потрібно увімкнути опцію "Enable the API to allow the ESP device to register and control entities in Home Assistant." [Демо відео](https://youtu.be/4dKZSFFEjWA).

---


## 🛠️ Зведена Таблиця Апаратного Забезпечення Lilka

| Компонент | GPIO / Призначення | Опис / Функція |
| :--- | :--- | :--- |
| **Платформа** | ESP32-S3 DevKitC-1 | Мікроконтролер з підтримкою PSRAM (Octal, 80MHz). |
| **Дисплей (ST7789V)** | | TFT-дисплей (280x240, SPI) з обертанням 270°. |
| CS Pin | GPIO7 | Chip Select для SPI. |
| DC Pin | GPIO15 | Data/Command для дисплея. |
| CLK Pin (SPI) | GPIO18 | Тактовий сигнал SPI. |
| MOSI Pin (SPI) | GPIO17 | Дані Master Out Slave In для SPI. |
| **Живлення Дисплея** | GPIO46 | Керування живленням (увімкненням) дисплея. |
| **Аудіо (I2S)** | | Налаштування I2S для зовнішнього динаміка (DAC). |
| LRCLK Pin | GPIO1 | Left/Right Clock. |
| BCLK Pin | GPIO42 | Bit Clock. |
| DOUT Pin | GPIO2 | Data Out (дані для динаміка). |
| **Батарея** | ADC GPIO3 | Аналоговий вхід для вимірювання напруги батареї (множник 1.33). |
| **Кнопка Up** | 38 | Навігація вгору / Зміна фокусу LVGL (змінює `current_button_index`). |
| **Кнопка Down** | 41 | Навігація вниз / Зміна фокусу LVGL. |
| **Кнопка Select** | 0 | **Вибір / Дія**. Використовується для **перемикання** `switch.boiler_switch` HA. |
| **Кнопка Left** | 39 | Навігаційний елемент. |
| **Кнопка Right** | 40 | Навігаційний елемент. |
| **Кнопка Start** | 4 | Функціональна кнопка. |
| **Кнопки A, B, C, D**| 5, 6, 10, 9 | Функціональні кнопки. |


---

## 🚀 Встановлення та налаштування

1. Завантажте [.bin файл](https://github.com/sverdlyuk/Lilka-ESPHome/releases), або скомпілюйте його самостійно з файлу .yaml
2. Прошийте Лілку по кабелю за допомогою [ESPHome Web](https://web.esphome.io)
3. Знайдіть та підключіться до Wi-Fi мережі Лілки, щоб встановити свою
